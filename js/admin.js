...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const adminSidebarLogoutBtn = document.getElementById('adminSidebarLogoutBtn');
  const adminTopbarLogoutBtn = document.getElementById('adminTopbarLogoutBtn');
  ...
  function checkAuthSession() {
    const session = window.CholanAPI ? window.CholanAPI.getAdminSession() : null;
    if (session && session.token) {
      if (authOverlay) authOverlay.style.display = 'none';
      if (adminActiveBadge) adminActiveBadge.textContent = session.admin?.email || 'Admin Active';
      loadAllAdminData();
      return true;
    } else {
  ...
  if (adminLoginForm) {
    adminLoginForm.onsubmit = async (e) => {
      ...
      try {
        const res = await window.CholanAPI.loginAdmin(email, password);
        if (res && res.success) {
          if (loginFeedback) loginFeedback.innerHTML = '<span style="color:#4ade80;"><i class="fa-solid fa-check"></i> Access Granted. Entering Dashboard...</span>';
          setTimeout(() => {
            if (authOverlay) authOverlay.style.display = 'none';
            if (adminActiveBadge) adminActiveBadge.textContent = res.admin?.email || 'Admin Active';
            loadAllAdminData();
          }, 350);
        } else {
          if (loginFeedback) loginFeedback.innerHTML = `<span style="color:#f87171;"><i class="fa-solid fa-triangle-exclamation"></i> ${res.message || 'Invalid Credentials'}</span>`;
        }
      } catch (err) {
        if (loginFeedback) loginFeedback.innerHTML = `<span style="color:#f87171;"><i class="fa-solid fa-triangle-exclamation"></i> Login error: ${err.message}</span>`;
      } finally {
  ...
  function handleLogout() {
    if (confirm('Are you sure you want to sign out of the Admin Portal?')) {
      if (window.CholanAPI) window.CholanAPI.logoutAdmin();
      if (authOverlay) authOverlay.style.display = 'flex';
  ...
  if (adminSidebarLogoutBtn) adminSidebarLogoutBtn.onclick = handleLogout;
  if (adminTopbarLogoutBtn) adminTopbarLogoutBtn.onclick = handleLogout;
  ...
  if (changePasswordForm) {
    changePasswordForm.onsubmit = async (e) => {
      ...
      if (newPassword !== confirmPassword) {
        if (fb) fb.innerHTML = '<span style="color:#f87171;">New passwords do not match!</span>';
        return;
      }
      if (newPassword.length < 6) {
        if (fb) fb.innerHTML = '<span style="color:#f87171;">New password must be at least 6 characters!</span>';
        return;
      }

      const session = window.CholanAPI ? window.CholanAPI.getAdminSession() : null;
      const email = session?.admin?.email || 'admin@cholanholidays.com';

      if (fb) fb.innerHTML = '<span style="color:#facc15;"><i class="fa-solid fa-spinner fa-spin"></i> Updating password...</span>';

      try {
        const res = await window.CholanAPI.changeAdminPassword(email, currentPassword, newPassword);
        if (res && res.success) {
          if (fb) fb.innerHTML = '<span style="color:#4ade80;"><i class="fa-solid fa-check"></i> Password updated successfully!</span>';
          changePasswordForm.reset();
          setTimeout(() => { if (fb) fb.textContent = ''; }, 3500);
        } else {
          if (fb) fb.innerHTML = `<span style="color:#f87171;"><i class="fa-solid fa-triangle-exclamation"></i> ${res.message || 'Failed to update password'}</span>`;
        }
      } catch (err) {
